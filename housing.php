<?php
/*
Plugin Name: Chicago Housing Widget Powered By Everyblock 
Plugin URI: http://tirespider.com/
Description: Add a Chicago Housing Widget to a post or page.
Version: 2.0.2
Author: Jeff S
...
class ChicagohousingWidget extends WP_Widget {

	public static $metro = "chicago";
	public static $schema = "housing";
	
	public function ChicagohousingWidget()
	{
		$widget_ops = array('classname' => 'ChicagohousingWidget', 'description' => 'Embeds a Chicago Housing Widget' );
		$this->WP_Widget('ChicagohousingWidget', 'Chicago Housing Widget', $widget_ops);
	}
...
add_action('widgets_init', create_function('', 'return register_widget("ChicagohousingWidget");') );

function display_chicago_housing_widget($atts) {
	$atts = extract(shortcode_atts( array('width' => '300', 'height' => '500'), $atts, 'display_chicago_housing_widget'));
	
	$code = "<iframe src=\"" . plugins_url( 'widget.php', __FILE__ ) . "?metro=" . ChicagohousingWidget::$metro . "&schema=" . ChicagohousingWidget::$schema . "&width=" . $width . "&height=" . $height . "&url=" . ABSPATH . "\" width=\"". $width . "\" //height=\"" . $height . "\" frameborder='0' scrolling='no'></iframe>";
	 
	return get_html_for_embed_chicago_housing(stripslashes($code));
}

function get_html_for_embed_chicago_housing($embedCode) {
	return '<div class="widget_block">' . stripslashes($embedCode) .'</div>';  
}

add_shortcode('display_chicago_housing_widget', display_chicago_housing_widget);
